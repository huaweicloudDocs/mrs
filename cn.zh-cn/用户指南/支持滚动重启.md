# 支持滚动重启<a name="ZH-CN_TOPIC_0173397704"></a>
...
2.  单击“组件管理“，选择需要滚动重启的服务，进入服务页面。
    ...
    >针对MRS 2.0.1及之前版本，具体请参见[滚动重启服务](支持滚动重启-184.md#zh-cn_topic_0138965094_section1115494813176)。  
...
4.  输入管理员密码后，弹出“滚动重启服务“页面，勾选“仅重启配置过期的实例“，单击确认，开始滚动重启服务。
...
2.  单击“组件管理“，选择需要滚动重启的服务，进入服务页面。
    ...
    >![](public_sys-resources/icon-note.gif) **说明：**   
    >针对MRS 2.0.1及之前版本，具体请参见[滚动重启实例](支持滚动重启-184.md#zh-cn_topic_0138965094_section938837152120)。  

3.  在“实例“页签，勾选要重启的实例，单击“更多“，选择“滚动重启实例“。
4.  输入管理员密码后，弹出“滚动重启实例“页面，勾选“仅重启配置过期的实例“，单击确认，开始滚动重启实例。
5.  滚动重启任务完成后，单击“完成“。
...
2.  在页面右上角选择“管理操作 \> 滚动重启集群”。
    ...
    >![](public_sys-resources/icon-note.gif) **说明：**   
    >针对MRS 2.0.1及之前版本，具体请参见[滚动重启集群](支持滚动重启-184.md#zh-cn_topic_0138965094_section1787148152416)。  

    **图 4**  服务管理<a name="fig6325453142516"></a>  
...
<td class="cellrowborder" valign="top" width="75.44%" headers="mcps1.2.3.1.2 "><p id="p11775123217"><a name="p11775123217"></a><a name="p11775123217"></a><span>滚动重启实例批次执行失败容错次数，默认为0，即表示任意一个批次的实例重启失败后，滚动重启任务终止。</span>取值范围为0～214748367。</p>
</td>
</tr>
</tbody>
</table>
...
2.  单击“组件管理“，选择HBase，进入HBase服务页面。
    ...
    >![](public_sys-resources/icon-note.gif) **说明：**   
    >针对MRS 2.0.1及之前版本，具体请参见[典型场景操作步骤](支持滚动重启-184.md#zh-cn_topic_0138965094_section830817219322)。  

3.  单击“服务配置“页签，修改HBase某个参数并保存配置，在出现如下弹窗后，单击“确定“进行保存。

    **图 5**  保存配置<a name="fig8181745152016"></a>  
...
6.  在“服务状态“页签单击“更多“，选择“滚动重启服务“。
    ...
    **图 6**  服务状态-滚动重启<a name="fig1429712581880"></a>  
    ![](figures/服务状态-滚动重启.png "服务状态-滚动重启")

7.  输入管理员密码后，弹出“滚动重启服务“页面，勾选“仅重启配置过期的实例“，单击确认，开始滚动重启。

    **图 7**  配置滚动重启服务<a name="fig15979153817152"></a>  
    ![](figures/配置滚动重启服务.png "配置滚动重启服务")

8.  滚动重启任务完成后，单击“完成“。

    **图 8**  滚动重启任务完成<a name="fig17461932121619"></a>  
